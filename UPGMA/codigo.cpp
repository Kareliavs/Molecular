...
unordered_map<string, unordered_map<string,float> > matriz;
unordered_map<string, unordered_map<string,float> > matriz_global;
...
unordered_map<string, float > distancias;

unordered_map<string, vector<float> > mapaincial;
vector<string> cabeceras;
int num_clusters=0;
void imprimir2(unordered_map<string, unordered_map<string,float> > matriz)
{	
	/*cout<<endl;
	fs<<endl;
...
unordered_map<string, unordered_map<string,float> > agrupando(unordered_map<string, unordered_map<string,float> > matriz)
{
	...
	unordered_map<string, unordered_map<string,float> > salida;

	salida=matriz;
	salida.erase(cabecerasanterior[pos1]);
	salida.erase(cabecerasanterior[pos2]);
	for (unordered_map<string, unordered_map<string,float> >::iterator  iteradorgrande=salida.begin(); iteradorgrande!=salida.end(); ++iteradorgrande)
	 {
	...
	for(int i=0; i<tope; i++)
	{
		...
		if(entrada==1)
		{
			vector<string>tokens = split_iterator(cluster);
			vector<string>tokens2 = split_iterator(cabeceras[i]);
			for(int k=0;k<tokens.size();k++)
			{
				for(int l=0;l<tokens2.size();l++)
				{
					valor=valor + matriz_global[ tokens[k]+"*" ][tokens2[l]+"*"];
					//cout<<"> "<<tokens[k]+"*" +"*"<<"\t"<<tokens2[l]+"*"<<"\t"<<matriz_global[ tokens[k]+"*" ][tokens2[l]+"*"]<<endl;
				}				
			}
			//cout<<"divide entre: "<<tokens.size()*tokens2.size()<<endl;
			valor=valor/(tokens.size()*tokens2.size());
		}
		else
		{
			for(int j=0;j<cluster.size();j++)
			{
				...
				ss << cluster[j];
				ss >> target;
				//cout<< matriz_global[ target ][ cabeceras[i] ]<<" : \t"<<target<<"\t"<<cabeceras[i]<<endl;
   ...
   // cout<<"---------------------------------------------------------------------"<<endl;
	...
	tiempo =tiempo+time0;
	cout<<"HOLA 4: "<<tope<<endl;
	return salida;
}

void repetir(unordered_map<string, unordered_map<string,float> > matriz)
{
	while(cabeceras.size()>2 and cabeceras.size()!= maxiteraciones)
