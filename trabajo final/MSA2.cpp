...
pair<string,int> global(string a1,string b1)
{
	...
	*/					
	...
	for(int i=1;i<=m;i++)
	{	
		mat[i%2][0]=mat[(i-1)%2][0]-2;
		for(int j=1;j<=n;j++)
			{
				if(s[i-1]==t[j-1]) ///si son iguales
					a=mat[(i-1)%2][j-1]+1; /// sumo
				else			
					a=mat[(i-1)%2][j-1]-1; /// resto
				b=mat[(i-1)%2][j]-2;
				c=mat[(i)%2][j-1]-2;
				maxi=max(a,max(b,c));
				route[i][j]='0';
	...
	pair<string, int> rpta = make_pair(cadena_fin,mat[1][n]);
	t3=clock();
