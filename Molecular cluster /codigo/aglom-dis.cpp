...
{	int nonegativo=-1;
	...
	{	
		...
		{
			a=elegidos[cabeceras[i]];
			float ref=matriz[ cabeceras[i] ][inmediato_anterior[0]];
			for(int k=1;k<inmediato_anterior.size();k++)
			{
				if(tipo==1)ref= min(ref,matriz[ cabeceras[i] ][inmediato_anterior[k]]);
				else if(tipo==2)ref= max(ref,matriz[ cabeceras[i] ][inmediato_anterior[k]]);
				else if(tipo==3)ref=ref+matriz[ cabeceras[i] ][inmediato_anterior[k]];
				//cout<<"ref: "<<ref<<endl;
			}
			if(tipo==3) b=ref/inmediato_anterior.size();
			else b=ref;
			dif=a-b; //cout<<"DIFERENCIA; "<<a<<" - "<<b<<" = "<<dif<<endl;
			//cout<<endl<<cabeceras[i]<<"\t"<<a<<"\t"<<b<<"\t"<<dif;
		}
		if(dif>=0)
		{
...
void repetirdis()
{
	elegir();
	//disociativo();
	while(cabeceras.size()>0 and apartados.size()!= maxiteraciones-1)
	{   
		disociativo();
		//cout<<apartados.size()<<"!="<<maxiteraciones-1<<endl;
		...
		if(apartados.size()==50-1 or apartados.size()==100-1 or apartados.size()==150-1 or apartados.size()==200-1 or cabeceras.size()==1)
		{	
			cout<<apartados.size()+1<<"------------------------------------------------------"<<endl;
			fs<<apartados.size()+1<<"------------------------------------------------------"<<endl;
			for(int i =0;i< apartados.size();i++)
			{	cout<<"cluster "<<i+1<<" "<< apartados[i]<<endl;
				fs<<"cluster "<<i+1<<" "<< apartados[i]<<endl;
			}
			cout<<"cluster "<<maxiteraciones<<" ";
			fs<<"cluster "<<maxiteraciones<<" ";
			for(int i=0; i<cabeceras.size();i++)
			{	
				cout<<cabeceras[i];
				fs<<cabeceras[i];
